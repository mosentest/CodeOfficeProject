...
public interface IssueResolutionRepository extends JpaRepository<IssueResolution, Long> {
	...
	@Query("SELECT COUNT(r) = 0 FROM IssueResolution r WHERE r.enterprise = :enterprise AND r.name = :name AND r.id <> :id")
	public boolean isNameAvailable(@Param("enterprise") Enterprise enterprise, @Param("name") String name, @Param("id") Long id);

	@Query("SELECT r FROM IssueResolution r WHERE r.enterprise = :enterprise ORDER By r.resolutionOrder")
